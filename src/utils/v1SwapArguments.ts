...
import { TokenAmount, Token, Trade, TradeType } from '@uniswap/sdk'
 ...
 */
export default function v1SwapArguments(trade: Trade, options: Omit<any, 'feeOnTransfer'>): any {
  ...
  if (isExactIn) {
    ...
    if (inputETH) {
      ...
      const output = trade.outputAmount.token
